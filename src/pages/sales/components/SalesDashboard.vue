      ...
      <v-row dense class="mb-1">
        <v-col cols="12" sm="6" md="3">
          <v-card rounded="lg" elevation="1" class="pa-4">
            ...
            <div class="text-h5 font-weight-bold">{{ formatCurrency(today.total) }}</div>
        ...
        <v-col cols="12" sm="6" md="3">
          <v-card rounded="lg" elevation="1" class="pa-4">
            ...
            <div class="text-h5 font-weight-bold">{{ formatCurrency(week.total) }}</div>
        ...
        <v-col cols="12" sm="6" md="3">
          <v-card rounded="lg" elevation="1" class="pa-4">
            ...
            <div class="text-h5 font-weight-bold">{{ formatCurrency(month.total) }}</div>
        ...
        <v-col cols="12" sm="6" md="3">
          <v-card rounded="lg" elevation="1" class="pa-4">
            ...
            <div class="text-h5 font-weight-bold" :class="lowStockCount > 0 ? 'text-warning' : ''">
              {{ lowStockCount }}
            </div>
          ...
          <v-card rounded="lg" elevation="1">
            ...
            <v-list lines="one" :loading="loading">
              <v-list-item v-for="p in topProducts" :key="p.name">
                ...
                <template #append>
                  <span class="font-weight-medium">{{ formatCurrency(p.revenue) }}</span>
                </template>
              </v-list-item>
          ...
          <v-card rounded="lg" elevation="1">
            ...
            <v-list lines="one">
              <v-list-item v-for="c in byCashier" :key="c.name">
                ...
                <template #append>
                  <span class="font-weight-medium">{{ formatCurrency(c.total) }}</span>
                </template>
              </v-list-item>
