...
function openConfirmWithSkuCheck(item: any) {
  ...
  selectedPR.value = prStore.prs.find(pr => Number(pr.id) === Number(item.requisition_id)) ?? null
...
async function handleValidateAndSaveAll() {
  ...
  // Save all SKU changes for all items
  ...
  const savePromises = items.map(async (item) => {
    if (item.SKU !== null && item.SKU !== undefined) {
      try {
        ...
        console.error(`Error saving SKU for item ${item.id}:`, err)
      }
    }
  })
