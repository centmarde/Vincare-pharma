...
import { updateItemSku } from '@/stores/productsData'
...
const originalSkuValues = ref<Record<number, number | null>>({})
...
// Save SKU for a specific item using the CRUD function from productsData.ts
async function saveSku(itemId: number) {
  ...
  try {
    const result = await updateItemSku(itemId, newSku)
    ...
    // Toast is already shown in updateItemSku, but we revert the change on error
