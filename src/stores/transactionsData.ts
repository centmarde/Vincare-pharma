...
export const useTransactionsDataStore = defineStore('transactionsData', () => {
  ...
  const fetchTransactions = async (options: FetchTransactionsOptions = {}) => {
    ...
    if (po_no_not_null) {
      ...
      q = q
        .or('reference_no.ilike.PO%,po_no.ilike.PO%')
        .in('transaction_type', ['purchase_order', 'stock_in'])
    }
    ...
    if (search?.trim()) {
      ...
      q = q.or(
        `requisition_no.ilike.%${s}%,po_no.ilike.%${s}%,remarks.ilike.%${s}%,status.ilike.%${s}%`,
      )
    }
  ...
  async function fetchTransactionsCount(options: FetchTransactionsOptions = {}): Promise<number> {
    ...
    if (po_no_not_null)
      q = q.ilike('po_no', 'PO%').in('transaction_type', ['purchase_order', 'stock_in'])
  ...
  return {
    ...
    fetchPurchaseRequisitionsRPC,
    fetchPurchaseOrdersRPC,
