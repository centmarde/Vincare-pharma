-- gl_project_events: an expense's category is a GL account code.
--
-- The voucher and Add Expense pickers read the chart of accounts now, so
-- finance_details.category holds a code ('7050'). The slug map stays as a
-- fallback for rows written before the switch.
--
-- DEPLOY ORDER: run schema/20260919_finance_details_category_drop_check.sql
-- FIRST (the old CHECK rejects account codes), then this, then the app.
--
-- Carried forward from 20260817 with two changed statements. Diff
--   select prosrc from pg_proc where proname = 'gl_project_events';
-- before running, or a newer live definition is silently reverted.
