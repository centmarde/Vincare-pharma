-- gl_project_events: a replacement issued against a customer return clears
-- 2065 Customer Credits instead of claiming cash arrived.
--
-- ⚠️ BUILT FROM THE LIVE DEFINITION, dumped 2026-09-26 and DIFFED against
-- 20260925_gl_project_events_goods_return.sql: the two were byte-identical
-- after normalising comments and whitespace (11,654 tokens each). So this file
-- is that verified base plus ONE changed expression, marked below.
--
-- WHAT CHANGES, AND WHY
-- A return credits 2065 (the company owes the customer goods). The replacement
-- is recorded as an ordinary POS sale so it relieves stock and recognises
-- revenue exactly like any other — but it is settled by that credit, not by
-- money. The POS loop previously read:
--
--     v_cash := case when r.payment_method in ('cash','petty_cash')
--                    then '1010' else '1020' end;
--
-- which sends anything that is not cash to 1020 Cash in Bank. A replacement
-- would therefore have booked DR 1020 — recording money into the bank that
-- nobody paid — and left 2065 outstanding for ever. Now a payment_method of
-- 'customer_credit' debits 2065, which is what consumes the credit the return
-- created. Everything else in this function is unchanged.
--
-- pos_sale_details.payment_method carries no CHECK constraint (verified by
-- probe), so the value needs no schema change.
...
begin
  ...
  loop
    ...
    begin
      ...
      -- ── CHANGED ─────────────────────────────────────────────────────────
      -- 'customer_credit' means goods issued against a return, settled by the
      -- credit that return created. Debiting 2065 clears that obligation; the
      -- old else-branch would have debited 1020 and invented a bank deposit.
      v_cash := case
        when r.payment_method = 'customer_credit' then '2065'
